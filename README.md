...
Para executar a aplicação, é necessário iniciar o servidor. Podemos executar o comando `node src/index.js`, mas podemos gerar um script para deixar esse comando mais curto. No arquivo package.json, setar `"main": "src/index.js"` e criar um script que chama o nodemon, `"dev": "nodemon"`. Lembrando que o nodemon vai reiniciar o servidor automaticamente a cada alteração que eu salvar no meu arquivo.
